...
- **User uploads live under `frontend/public/art/`, organized per module** (mirroring `features/`): `art/turris/{librarian-portraits, librarian-previews, floors}` + `art/armarium/{plants, cards, backgrounds}` + per-module `_trash/`. Never write new uploads to the `art/` root. `trashArt()` routes trash to the owning module's `_trash/`.
...
- **Adding a new battle system** (STS, NASE, ...): add the id to the `BattleSystemId` union and an entry to `BATTLE_SYSTEMS` in `shared/src/index.ts` — that single change automatically enables it in the librarian editor's system dropdown, `emptySheet()` templates, `speedPassiveTemplates()` fallback, and per-system editor sections. No other wiring is required.
