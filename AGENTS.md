...
- Turris term rendering is async: `terms/renderer.ts` exposes `ensureTermIndex()` / `termIndexReady()` / `renderTermText()`; data comes from the backend via `store/terms.ts` (Pinia). Static `terms/data/*.ts` files are import seeds only — after editing them, run `npm run import:terms` to re-import into SQLite. `shared/src/terms/*` (cardTypes/baseTags/cardPrefixes/mechanism etc.) are NOT DB-backed: they feed `TermInserter`'s insert palette directly.
