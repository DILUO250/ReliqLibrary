...
/** 两级查表：exact = 精确键（保留【】分支名），loose = 宽松键（历史 baseKey）。 */
let EXACT_MAP: Map<string, DictEntry> = new Map()
let LOOSE_MAP: Map<string, DictEntry> = new Map()
/** 仅用于 termIndexReady() 的"库里有没有词条"判定。 */
let INDEX: unknown[] = []
...
function rebuildIndex(): void {
  ...
  const maps = buildLookupMaps(entries)
...
/**
 * 解析某个被引号包裹的词条 → 字体格式。
 * 链：私人（全等 → exactKey → 无括号宽松兜底）→ 全局精确键 → 全局宽松键 → 组合解析。
 */
function resolveFormat(q: string, privateTerms: PrivateTerm[], privateKey: string): TermFormat | undefined {
  const p = matchPrivateTerm(q, privateTerms)
  if (p) return p.format
  void privateKey
  const e = EXACT_MAP.get(exactKeyCached(q))
  if (e) return mergedFormat(e)
  const g = LOOSE_MAP.get(baseKeyCached(q))
 ...
 * 注意：依赖模块级 EXACT_MAP / LOOSE_MAP（非响应式），调用方必须配合 termIndexReady() 做双态渲染。
 */
