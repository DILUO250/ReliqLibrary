...
function slLabel(sl: number): string {
  // 与 shared 的 parseSpaceReport 同一 clamp 规则（1~99），保证屏显与回读一致
  return `SL-${String(Math.min(99, Math.max(1, Math.trunc(Number(sl)) || 1))).padStart(2, '0')}`
}

/* ---------- 行模型（分页渲染的原子单位） ----------
 * 类型来自 shared/paper（通用引擎）；此处只负责 SCL 空间报告单的内容格式：
 * 每行可独立测高、不可跨页拆分；页组装时把连续 item 行重组成 <ul class="r-list">，
 * quoted 文件行按 group 在页内重组 .file-box 边框。
 * 受限区为区间覆盖制（shared 的 SpaceWarningAnchor 注释）：警告行深度恒 0（区间边界），
 * 区内行深度恒 1、restrictedSls=[governing sl]，永不嵌套。 */

export type SpaceRowKind = 'head' | 'section' | 'item' | 'entry' | 'figure' | 'warn'
 ...
 */
function anchorRank(anchor: SpaceWarningAnchor): number {
  const entry = /^entry:(\w+):(\d+)$/.exec(anchor)
  if (!entry) {
    switch (anchor) {
      case 'block:head': return 0
      case 'block:desc': return 100
      case 'block:entry-exit': return 150
      case 'block:phenomena': return 160
      case 'block:entity-dist': return 170
...
/**
 * 空间报告单 → 原子行序列（区块序 = 模板 docx：描述→入口与出口→现象分布→
 * 实体分布→控制措施→附录→文件→影像资料→附件；警告线区间覆盖制同实体版）。
 */
export function buildSpaceRows(space: SupernaturalSpace, report: SpaceReport): ReportRow[] {
  const code = (space.code || '').replace(/\D/g, '') || 'XXXX'
  const na = '<span class="na">N/A</span>'

  const headRow: ReportRow = {
    kind: 'head',
    html:
      `<h2 class="r-title">${esc(anomalyReportTitle(space))}</h2>` +
      '<div class="r-meta">' +
      `<p>项目编号：SCL-${esc(code)}</p>` +
      `<p>项目等级：<strong>${esc(spaceLevelText(space.level, space.subLevel))}</strong></p>` +
      '</div>',
    keepWithNext: false,
    group: null,
    restrictedDepth: 0,
  ...
  const sectionRow = (label: string, anno: string): ReportRow => ({
    kind: 'section',
    html: `<h3 class="r-h">${esc(label)}<span class="r-anno">${esc(anno)}</span></h3>`,
    keepWithNext: true,
    group: null,
    restrictedDepth: 0,
    restrictedSls: [],
  ...
  const entryRow = (kindLabel: string, idxLabel: string, source: string): ReportRow => ({
    kind: 'entry',
    html:
  ...
  // 与 shared 的 spaceWarningAnchors 同一文档序：anchor 列表 = 编辑器下拉 = 回退链。
  const rows: ReportRow[] = []
  const landmarks = new Map<SpaceWarningAnchor, number>()
  const push = (row: ReportRow, anchor?: SpaceWarningAnchor): void => {
  ...
  for (const t of report.description.length ? report.description : ['']) {
    rows.push(itemRow(t ? `<li>${esc(t)}</li>` : naItem()))
  }

  push(sectionRow('入口与出口：', 'ENTRANCE & EXIT'), 'block:entry-exit')
  for (const t of report.entry.length ? report.entry : ['']) {
    rows.push(itemRow(t ? `<li><strong>入口</strong>：${esc(t)}</li>` : `<li><strong>入口</strong>：${na}</li>`))
  }
  for (const t of report.exit.length ? report.exit : ['']) {
    rows.push(itemRow(t ? `<li><strong>出口</strong>：${esc(t)}</li>` : `<li><strong>出口</strong>：${na}</li>`))
  }

  push(sectionRow('超自然现象分布：', 'PHENOMENA'), 'block:phenomena')
  if (report.phenomena.length) {
    report.phenomena.forEach((p, i) => {
      const name = p.name ? `${esc(p.name)}：` : ''
      rows.push(itemRow(`<li>${i + 1}. ${name}${esc(p.desc)}</li>`))
    })
  } else {
    rows.push(itemRow(naItem()))
  }

  push(sectionRow('异常实体分布：', 'ENTITY DISTRIBUTION'), 'block:entity-dist')
  if (report.entityDistribution.length) {
    report.entityDistribution.forEach((p, i) => {
      const name = p.name ? `${esc(p.name)}：` : ''
      rows.push(itemRow(`<li>${i + 1}. ${name}${esc(p.desc)}</li>`))
    })
  } else {
  ...
  push(sectionRow('特殊控制措施：', 'SPECIAL CONTROL'), 'block:contain')
  for (const t of report.containment.length ? report.containment : ['']) {
    rows.push(itemRow(t ? `<li>${esc(t)}</li>` : naItem()))
  }

  report.appendices.forEach((a, i) => {
    rows.push(entryRow('附录', String(i + 1), a.source))
  ...
  report.files.forEach((f, i) => {
    rows.push(entryRow('文件', anomalyFileIndexLabel(i), f.source))
  ...
  report.attachments.forEach((a, i) => {
    rows.push(entryRow('附件', String(i + 1), ''))
    landmarks.set(`entry:attachment:${i + 1}`, rows.length - 1)
    for (const t of a.body) rows.push(itemRow(`<li>${esc(t)}</li>`))
  })

  /* ---------- 警告线解析与插入（区间覆盖制，逻辑与实体版同构） ---------- */

  // 锚点 → 行号：精确命中；条目越界 / 区块为空 → 沿已渲染插入点找文档序最近前驱
  //（与 shared 的 spaceWarningAnchors 同一顺序，"安全分级不能丢"）。
  const resolveRowIdx = (anchor: SpaceWarningAnchor): number => {
 ...
 * 纯函数：同样输入永远输出同样 HTML。（与实体版 composePageHtml 同构，印章延迟渲染。）
 */
export function composeSpacePageHtml(rows: ReportRow[], start: number, end: number): string {
  let html = ''
  let opened = 0
