/**
 * 藏书阁超自然空间报告单 · 服务端 PDF 导出（双段式接口，镜像 exportRoutes.ts）：
 *
 * ① POST /api/armarium/export/space/:id（带 x-rtl-key）——同步阻塞：
 *    查库取空间 → 无头浏览器加载前端 /print 无 UI 打印路由 → 等分页完成信号 →
 *    printToPDF 落盘 export-cache/<UUID>.pdf → 返回 { id, filename }。
 * ② GET /api/armarium/export/space/file/:uuid（GET 不设防）——
 *    UUID 随机不可枚举 = "持链接者得"的能力令牌；application/pdf + attachment 原生下载。
 *
 * 暂存语义与 anomaly 导出同一目录/TTL（export-cache/ + sweepExpired 同款）。
 */
...
import { spaceExportFilename } from '@rtl/shared'
...
export async function registerSpaceExportRoutes(app: FastifyInstance): Promise<void> {
  // ① 触发导出（同步阻塞：生成约 3~5s，前端按钮期间显示"生成中"）
  app.post('/api/armarium/export/space/:id', async (req: FastifyRequest<{ Params: { id: string } }>, reply) => {
    const { id } = req.params
    const row = getDb()
      .prepare('SELECT code, name FROM supernatural_spaces WHERE id = ?')
      .get(id) as { code: string; name: string } | undefined
    if (!row) return reply.code(404).send({ error: '未找到该超自然空间档案' })

    await fs.mkdir(EXPORT_CACHE_DIR, { recursive: true })
    await sweepExpired()

    const uuid = randomUUID()
    const printUrl = `${FRONTEND_URL}/print/armarium/space/${encodeURIComponent(id)}`
    try {
      const { pageCount } = await printUrlToPdf(printUrl, cacheFile(uuid))
      req.log.info({ id, pageCount, printUrl }, 'space report exported to pdf')
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e)
      req.log.error({ id, printUrl, err: msg }, 'space pdf export failed')
      return reply.code(502).send({ error: `PDF 生成失败：${msg}` })
    }

    const filename = spaceExportFilename(row)
  ...
  app.get('/api/armarium/export/space/file/:uuid', async (req: FastifyRequest<{ Params: { uuid: string } }>, reply) => {
    ...
    const filename = `${entry?.filename ?? 'SCL空间报告单'}.pdf`
    reply.type('application/pdf')
    // 中文文件名：RFC 5987 filename* 编码 + ASCII 兜底
    reply.header(
      'Content-Disposition',
      `attachment; filename="SCL-space-report.pdf"; filename*=UTF-8''${encodeURIComponent(filename)}`,
    )
